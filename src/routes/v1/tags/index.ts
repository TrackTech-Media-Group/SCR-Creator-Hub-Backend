...
type Filter = ContentTypeFilter | "force";
...
export default class extends Route<CreatorHubServer> {
	public [methods.GET](req: Request, res: Response) {
		...
		res.json(type === "force" ? tags : tags.filter((tag) => tag.hasType(type)));
	}
	...
	private parseQuery(query: Record<string, any>) {
		...
		const type: Filter = typeof _type === "string" && [...CONTENT_TYPE_FILTER, "force"].includes(_type as any) ? (_type as any) : "all";
